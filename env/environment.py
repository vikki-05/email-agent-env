...
class EmailEnv:
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:
        ...
        # Check if episode ended via action (e.g. proper close)
        obs = self._make_observation()
        done = s.is_done()

        if done:
            # Episode is over — no further penalties
            info["resolved"] = s.resolved
            if not s.resolved and s.steps >= 5:
                info["step_limit_exceeded"] = True
            return obs, reward, done, info

        # --- Penalties (only applied while episode is still active) ---
        # Waiting too long without escalation — applied once per episode
    ...
    def _reward_reply(action: Action, s: EmailState) -> float:
        """Score reply quality via keyword overlap with the true intent."""
        keywords = INTENT_KEYWORDS.get(s.true_intent, [])
        content = (action.content or "").lower()
        if not content:
            return EmailEnv.REWARD_BAD_REPLY  # empty reply
    ...
    def _reward_close(self, action: Action, s: EmailState) -> float:
        """Closing is 'proper' if the agent classified correctly and replied/escalated."""
        ...
        if not classifications:
            return self.REWARD_EARLY_CLOSE  # never classified

        # Check if ANY classification was correct (not all wrong)
        ...
        # Closing early without proper handling
...
if __name__ == "__main__":
    ...
    # Simulate a good trajectory
    actions = [
        Action("classify", "refund_request"),  # correct
        Action("reply", "I can help process your refund for the charged amount."),
        Action("close"),  # should succeed
    ]
    ...
    print(f"  Steps taken : {env.state.steps}")
    print(f"  History     : {env.state.history}")
    print(f"{'=' * 60}")

    # --- Bad trajectory demo ---
    print("\n--- Bad trajectory demo ---")
    obs = env.reset()
    true_intent = env.state.true_intent
    bad_actions = [
        Action("classify", "billing_inquiry"),  # likely wrong
        Action("close"),  # closing early
    ]
    total_reward = 0.0
    for i, act in enumerate(bad_actions, 1):
        obs, rew, done, info = env.step(act)
        total_reward += rew
        correct_label = "correct" if act.content == true_intent else "wrong"
        print(f"\n[Step {i}] Action: {act.action_type} | content={act.content} ({correct_label}, true={true_intent})")
        print(f"  Reward: {rew:+.2f} | Done: {done} | Info: {info}")
        if done:
            break
    ...
    # --- Edge case: step limit exceeded ---
    print("\n--- Step limit exceeded demo ---")
    obs = env.reset()
    total_reward = 0.0
    for i in range(7):
        obs, rew, done, info = env.step(Action("classify", "wrong_intent"))
        total_reward += rew
        print(f"\n[Step {i+1}] Action: classify | Reward: {rew:+.2f} | Done: {done} | Info: {info}")
        if done:
            break
    ...
    # --- Edge case: escalation on high priority ---
    print("\n--- Escalation on high priority demo ---")
    env = EmailEnv(seed=0)
    obs = env.reset()
    print(f"  True intent: {env.state.true_intent} | Priority: {env.state.priority}")
    obs, rew, done, info = env.step(Action("escalate"))
    print(f"  Escalate | Reward: {rew:+.2f} | Done: {done} | Info: {info}")

    print(f"\n{'=' * 60}")
    print("\nAll tests passed — environment runs without errors.")
